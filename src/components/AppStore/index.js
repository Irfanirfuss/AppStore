...
class AppStore extends Component {
  ...
  getActiveTabApps = results => {
    const {searchInput} = this.state
    const filteresults = results.filter(each =>
  ...
  getSearchResults = id => {
    const {searchInput} = this.state
    const filterApp = appsList.filter(each => each.category === id)
    const searchResults = filterApp.filter(each =>
      each.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
  ...
  render() {
    ...
    const searchResults = this.getSearchResults(activeTabId)
